...
if ( ! function_exists( 'eldon_core_add_mobile_header_options' ) ) {
	/**
	 * Function that add general options for this module
	 */
	function eldon_core_add_mobile_header_options() {
		...
		$page = $qode_framework->add_options_page(
			array(
				...
				'slug'        => 'mobile-header',
				'icon'        => 'fa fa-cog',
				'title'       => esc_html__( 'Mobile Header', 'eldon-core' ),
				'description' => esc_html__( 'Global Mobile Header Options', 'eldon-core' ),
			)
		);
		...
		if ( $page ) {
			...
			$general_tab->add_field_element(
				array(
					...
					'title'         => esc_html__( 'Sticky Mobile Header', 'eldon-core' ),
					'description'   => esc_html__( 'Set mobile header to be sticky', 'eldon-core' ),
				)
			);
			...
			$general_tab->add_field_element(
				array(
					...
					'options'       => apply_filters( 'eldon_core_filter_mobile_header_layout_option', array() ),
				)
			);
			...
			$section_svg_path->add_field_element(
				array(
					...
					'title'       => esc_html__( 'Mobile Menu Open Icon SVG Path', 'eldon-core' ),
					'description' => esc_html__( 'Enter your mobile menu open icon SVG path here. Please remove version and id attributes from your SVG path because of HTML validation', 'eldon-core' ),
				)
			);
			...
			$section_svg_path->add_field_element(
				array(
					...
					'title'       => esc_html__( 'Mobile Menu Close Icon SVG Path', 'eldon-core' ),
					'description' => esc_html__( 'Enter your mobile menu close icon SVG path here. Please remove version and id attributes from your SVG path because of HTML validation', 'eldon-core' ),
				)
			);
			...
			$opener_section = $general_tab->add_section_element(
				array(
					...
					'title' => esc_html__( 'Mobile Header Opener Styles', 'eldon-core' ),
				)
			);
			...
			$opener_section_row->add_field_element(
				array(
					...
					'title'      => esc_html__( 'Hover/Active Color', 'eldon-core' ),
					'args'       => array(
						'col_width' => 3,
					),
				)
			...
			do_action( 'eldon_core_action_after_mobile_header_options_map', $page, $general_tab );
		}
	}

	add_action( 'eldon_core_action_default_options_init', 'eldon_core_add_mobile_header_options', eldon_core_get_admin_options_map_position( 'mobile-header' ) );
}
