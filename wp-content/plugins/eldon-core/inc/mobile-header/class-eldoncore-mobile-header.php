...
abstract class EldonCore_Mobile_Header {
	...
	public function __construct() {
		...
		add_filter( 'eldon_core_filter_nav_menu_mobile_header_selector', array( $this, 'set_nav_menu_header_selector' ) );
		add_filter( 'eldon_filter_mobile_header_inner_class', array( $this, 'set_mobile_header_inner_classes' ), 10, 2 );
		add_filter( 'eldon_filter_add_inline_style', array( $this, 'set_inline_mobile_header_styles' ) );
		add_filter( 'eldon_core_filter_content_margin_mobile', array( $this, 'get_content_margin' ) );
		add_filter( 'eldon_core_filter_title_padding_mobile', array( $this, 'get_title_padding' ) );
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'eldon_core_filter_mobile_header_template', $parameters );

		return eldon_core_get_template_part( 'mobile-header/layouts/' . $this->get_layout(), 'templates/' . $this->get_layout(), $this->get_layout_slug(), $parameters );
	}
	...
	function set_mobile_header_inner_classes( $class, $layout ) {
		...
		$class[] = 'yes' === eldon_core_get_post_value_through_levels( 'qodef_mobile_header_in_grid' ) ? 'qodef-content-grid' : '';
	...
	public function set_inline_mobile_header_styles( $style ) {
		...
		$height           = eldon_core_get_post_value_through_levels( 'qodef_' . $this->get_layout() . '_mobile_header_height' );
		$background_color = eldon_core_get_post_value_through_levels( 'qodef_' . $this->get_layout() . '_mobile_header_background_color' );
		...
		$side_padding = eldon_core_get_post_value_through_levels( 'qodef_' . $this->get_layout() . '_mobile_header_side_padding' );
	...
	function set_header_height() {
		$header_height = eldon_core_get_post_value_through_levels( 'qodef_' . $this->get_layout() . '_mobile_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'eldon_core_filter_set_mobile_header_height', $header_height );
	}
