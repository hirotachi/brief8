...
class EldonCore_Top_Area {
	...
	public function __construct() {
		...
		add_action( 'eldon_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'eldon_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
		add_filter( 'eldon_core_filter_top_area_inner_class', array( $this, 'set_grid_class' ) );
	}

	/**
	 * @return EldonCore_Top_Area
	 */
	...
	function is_top_area_enabled() {
		$top_area_enabled = 'yes' === eldon_core_get_post_value_through_levels( 'qodef_top_area_header' );
	...
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = eldon_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
	...
	public function is_top_area_transparent() {
		$background_color = eldon_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		...
		eldon_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles( $style ) {
		...
		$height       = eldon_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
		$side_padding = eldon_core_get_post_value_through_levels( 'qodef_top_area_header_side_padding' );
		$border_color = eldon_core_get_post_value_through_levels( 'qodef_top_area_header_border_color' );
		$border_width = eldon_core_get_post_value_through_levels( 'qodef_top_area_header_border_width' );
		$border_style = eldon_core_get_post_value_through_levels( 'qodef_top_area_header_border_style' );
	...
	public function set_grid_class( $classes ) {
		if ( 'yes' === eldon_core_get_post_value_through_levels( 'qodef_top_area_header_in_grid' ) ) {
...
EldonCore_Top_Area::get_instance();
