...
if ( ! function_exists( 'eldon_core_add_bottom_header_options' ) ) {
	function eldon_core_add_bottom_header_options( $page, $general_header_tab ) {

		$section = $general_header_tab->add_section_element(
			array(
				'name'        => 'qodef_bottom_header_section',
				'title'       => esc_html__( 'Bottom Header', 'eldon-core' ),
				'description' => esc_html__( 'Bottom header settings', 'eldon-core' ),
				'dependency'  => array(
					'show' => array(
		...
		if ( qode_framework_is_installed( 'revolution-slider' ) ) {
			$section->add_field_element(
				array(
					...
					'default_value' => '',
					'options'       => eldon_core_get_select_type_options_pool( 'rev_sliders', false ),
				)
			);
		}
	...
	add_action( 'eldon_core_action_after_header_options_map', 'eldon_core_add_bottom_header_options', 10, 2 );
}