...
if ( ! function_exists( 'eldon_core_add_portfolio_single_sidebar_meta_boxes' ) ) {
	/**
	 * Function that add general meta box options for this module
	 *
	 * @param object $page
	 */
	function eldon_core_add_portfolio_single_sidebar_meta_boxes( $page ) {
		...
		if ( $page ) {
			...
			$sidebar_tab = $page->add_tab_element(
				array(
					...
					'description' => esc_html__( 'Portfolio sidebar settings', 'eldon-core' ),
				)
			);
	...
	add_action( 'eldon_core_action_after_portfolio_meta_box_map', 'eldon_core_add_portfolio_single_sidebar_meta_boxes', 12 );
}
