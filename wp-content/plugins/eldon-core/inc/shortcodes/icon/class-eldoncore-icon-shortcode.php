...
if ( ! function_exists( 'eldon_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function eldon_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'EldonCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'eldon_core_filter_register_shortcodes', 'eldon_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'EldonCore_Shortcode' ) ) {
	class EldonCore_Icon_Shortcode extends EldonCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( ELDON_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'eldon_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'eldon-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'eldon-core' ) );
			...
			$this->set_option(
				array(
					'field_type' => 'iconpack',
					'name'       => 'main_icon',
					'title'      => esc_html__( 'Icon', 'eldon-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'eldon-core' ),
						'small'   => esc_html__( 'Small', 'eldon-core' ),
						'medium'  => esc_html__( 'Medium', 'eldon-core' ),
						'large'   => esc_html__( 'Large', 'eldon-core' ),
						'huge'    => esc_html__( 'Huge', 'eldon-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'eldon-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'eldon-core' ),
					'group'      => esc_html__( 'Style', 'eldon-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'eldon-core' ),
					'group'      => esc_html__( 'Style', 'eldon-core' ),
				)
			);
			$this->set_option(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'eldon_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return eldon_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
