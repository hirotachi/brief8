...
if ( ! function_exists( 'eldon_core_add_icon_list_item_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function eldon_core_add_icon_list_item_shortcode( $shortcodes ) {
		$shortcodes[] = 'EldonCore_Icon_List_Item_Shortcode';

		return $shortcodes;
	}

	add_filter( 'eldon_core_filter_register_shortcodes', 'eldon_core_add_icon_list_item_shortcode' );
}

if ( class_exists( 'EldonCore_Shortcode' ) ) {
	class EldonCore_Icon_List_Item_Shortcode extends EldonCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( ELDON_CORE_SHORTCODES_URL_PATH . '/icon-list-item' );
			$this->set_base( 'eldon_core_icon_list_item' );
			$this->set_name( esc_html__( 'Icon List Item', 'eldon-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon list item element', 'eldon-core' ) );
			$this->set_category( esc_html__( 'Eldon Core', 'eldon-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'item_margin_bottom',
					'title'      => esc_html__( 'Item Margin Bottom', 'eldon-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'eldon_core_icon_list_item', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return eldon_core_get_template_part( 'shortcodes/icon-list-item', 'templates/icon-list-item', '', $atts );
		}
