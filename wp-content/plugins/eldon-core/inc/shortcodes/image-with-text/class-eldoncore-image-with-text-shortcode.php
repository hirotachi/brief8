...
if ( ! function_exists( 'eldon_core_add_image_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function eldon_core_add_image_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'EldonCore_Image_With_Text_Shortcode';

		return $shortcodes;
	}

	add_filter( 'eldon_core_filter_register_shortcodes', 'eldon_core_add_image_with_text_shortcode' );
}

if ( class_exists( 'EldonCore_Shortcode' ) ) {
	class EldonCore_Image_With_Text_Shortcode extends EldonCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'eldon_core_filter_image_with_text_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'eldon_core_filter_image_with_text_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( ELDON_CORE_SHORTCODES_URL_PATH . '/image-with-text' );
			$this->set_base( 'eldon_core_image_with_text' );
			$this->set_name( esc_html__( 'Image With Text', 'eldon-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image with text element', 'eldon-core' ) );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'eldon_core_image_with_text', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return eldon_core_get_template_part( 'shortcodes/image-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
