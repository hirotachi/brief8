...
if ( ! function_exists( 'eldon_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function eldon_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'EldonCore_Highlight_Shortcode';

		return $shortcodes;
	}

	add_filter( 'eldon_core_filter_register_shortcodes', 'eldon_core_add_highlight_shortcode' );
}

if ( class_exists( 'EldonCore_Shortcode' ) ) {
	class EldonCore_Highlight_Shortcode extends EldonCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( ELDON_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'eldon_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'eldon-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'eldon-core' ) );
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'text_tag',
					'title'         => esc_html__( 'Text Tag', 'eldon-core' ),
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Disable Text Line Break', 'eldon-core' ),
					'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'eldon-core' ),
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Highlight Text Position', 'eldon-core' ),
					'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'eldon-core' ),
					'group'       => esc_html__( 'Text Style', 'eldon-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Color', 'eldon-core' ),
					'group'      => esc_html__( 'Text Style', 'eldon-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Background Color', 'eldon-core' ),
					'group'      => esc_html__( 'Text Style', 'eldon-core' ),
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return eldon_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
