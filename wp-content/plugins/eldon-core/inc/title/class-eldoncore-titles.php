...
class EldonCore_Titles {
	...
	public function __construct() {
		...
		add_filter( 'eldon_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );

		// Add title area inline styles
		add_filter( 'eldon_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}

	/**
	 * @return EldonCore_Titles
	 */
	...
	function include_layouts() {
		...
		foreach ( glob( ELDON_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( ELDON_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = eldon_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'eldon_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'eldon_filter_title_template' : 'eldon_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = eldon_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = eldon_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = eldon_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$image_params       = eldon_core_get_page_title_image_params();
	...
	function add_inline_styles( $style ) {
		...
		$height                   = eldon_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile            = eldon_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_top_margin         = eldon_core_get_post_value_through_levels( 'qodef_page_title_top_margin' );
		$title_bottom_margin      = eldon_core_get_post_value_through_levels( 'qodef_page_title_bottom_margin' );
		$title_padding            = apply_filters( 'eldon_core_filter_title_padding', 0 );
		$title_padding_mobile     = apply_filters( 'eldon_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = eldon_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color         = eldon_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params             = eldon_core_get_page_title_image_params();
		...
		if ( ! empty( $title_top_margin ) ) {
			$styles['margin-top'] = intval( $title_top_margin ) . 'px';
		}

		if ( ! empty( $title_bottom_margin ) ) {
			$styles['margin-bottom'] = intval( $title_bottom_margin ) . 'px';
		}
		...
		$title_color = eldon_core_get_post_value_through_levels( 'qodef_page_title_color' );
