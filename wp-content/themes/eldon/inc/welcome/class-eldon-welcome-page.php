...
if ( ! class_exists( 'Eldon_Welcome_Page' ) ) {
	/**
	 * Welcome page theme class
	 */
	class Eldon_Welcome_Page {
		private static $instance;

		/**
		 * @return Eldon_Welcome_Page
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_eldon_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_eldon_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_eldon_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'eldon_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			...
			remove_submenu_page( 'themes.php', 'eldon_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( ELDON_ROOT_DIR . '/screenshot.png' ) ? ELDON_ROOT . '/screenshot.png' : ELDON_ROOT . '/screenshot.jpg';

			eldon_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_eldon_welcome_page' === $hook ) {
				wp_enqueue_style( 'eldon-welcome-page-style', ELDON_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Eldon_Welcome_Page::get_instance();
}
