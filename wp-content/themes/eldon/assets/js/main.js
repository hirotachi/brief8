(function ( $ ) {
	...
	var qodefSwiper = {
		...
		getOptions: function ( $holder, returnBreakpoints ) {
			...
			if ( autoplay !== false && reverseDirection !== false ) {
				if ( typeof autoplay !== "boolean" ) {
					autoplay.reverseDirection = reverseDirection;
				} else {
					autoplay = {
						disableOnInteraction: false,
			...
			if ( ! customStages ) {
				if ( slidesPerView < 2 ) {
					...
					slidesPerView1024 = slidesPerView;
					slidesPerView768  = slidesPerView;
					slidesPerView680  = slidesPerView;
					slidesPerView480  = slidesPerView;
				}
			}
			...
			return {
				on: {
					init: function () {
						...
						qodef.body.trigger(
							'eldon_trigger_swiper_is_initialized',
							[$holder, options]
						);
	...
	var qodefImageBlur = {
		init: function () {
			var $gallery = $('.qodef-portfolio-list.qodef-image-blurred, .qodef-woo-product-list'),
				uniueNum = 0;
			
			if ( $gallery.length ) {
				$gallery.each(function () {
					var $thisGallery = $(this),
						$items = $thisGallery.find('.qodef-e, .qodef-woo-product-inner');
					
					$items.each(function () {
						uniueNum ++;
						
						var item = $(this);
						if (!item.find('.qodef-svg-blur-filter').length) {
							qodefImageBlur.appendSvg( item, uniueNum);
						}
						
						var image = item.find('.qodef-e-media-image img, .qodef-woo-product-image img');
						var svg = item.find('.qodef-svg-blur-filter'),
							filter = svg.find('filter'),
							filterId = filter.attr("id"),
							gaussianBlur = filter.find('feGaussianBlur')[0],
							stdDeviation = { val: 0};
						
						if ($gallery.hasClass('qodef-woo-product-list')){
							var imgScr = image[0].src;
							
							image.parent().css('background-image','url('+ imgScr +')');
							console.log(imgScr);
						}
						...
						item.timeline = gsap.timeline({
							...
							onUpdate: () => {
								gaussianBlur.setAttribute('stdDeviation', stdDeviation.val)
							}
						})
						
						item.timeline.to(stdDeviation, {
							startAt: {
								val: 0
							},
							val: 7,
						...
						qodefImageBlur.initEvents( item );
					});
				});
			}
		},
		appendSvg(item, uniueNum) {
			var blurFilterSvg = '<svg class="qodef-svg-blur-filter" width="100%" height="100%">' +
				'<filter id="qodef-svg-blur-' + uniueNum + '" x="-25%" y="-25%" width="150%" height="150%">' +
				'<feGaussianBlur in="SourceGraphic" stdDeviation="0" />' +
				'</filter></svg>';
			
			item.append(blurFilterSvg);
		},
		initEvents( holder ) {
			holder[0].addEventListener('mouseenter', function () {
					holder.timeline.restart();
				}
			);
			holder[0].addEventListener('mouseleave', function () {
				holder.timeline.reverse();
			});
		}
	};
	qodef.qodefImageBlur = qodefImageBlur;
