...
if ( ! class_exists( 'Eldon_Handler' ) ) {
	...
	class Eldon_Handler {
		...
		public function __construct() {
			...
			require_once ELDON_ROOT_DIR . '/helpers/helper.php';
			...
			add_action( 'eldon_action_before_main_css', array( $this, 'include_plugins_styles' ) );

			// Include theme's 3rd party plugins scripts
			add_action( 'eldon_action_before_main_js', array( $this, 'include_plugins_scripts' ) );

			// Add pingback header
			add_action( 'wp_head', array( $this, 'add_pingback_header' ), 1 );

			// Include theme's skip link
			add_action( 'eldon_action_after_body_tag_open', array( $this, 'add_skip_link' ), 5 );

			// Include theme's Google fonts
			add_action( 'eldon_action_before_main_css', array( $this, 'include_google_fonts' ) );
		...
		function include_css_scripts() {
			...
			$main_css_dependency = apply_filters( 'eldon_filter_main_css_dependency', array( 'swiper' ) );

			// Hook to include additional scripts before theme's main style
			do_action( 'eldon_action_before_main_css' );

			// Enqueue theme's main style
			wp_enqueue_style( 'eldon-main', ELDON_ASSETS_CSS_ROOT . '/main.min.css', $main_css_dependency );

			// Enqueue theme's style
			wp_enqueue_style( 'eldon-style', ELDON_ROOT . '/style.css' );

			// Hook to include additional scripts after theme's main style
			do_action( 'eldon_action_after_main_css' );
		}

		function add_inline_style() {
			$style = apply_filters( 'eldon_filter_add_inline_style', $style = '' );

			if ( ! empty( $style ) ) {
				wp_add_inline_style( 'eldon-style', $style );
			}
		}

		function include_js_scripts() {
			// JS dependency variable
			$main_js_dependency = apply_filters( 'eldon_filter_main_js_dependency', array( 'jquery' ) );

			// Hook to include additional scripts before theme's main script
			do_action( 'eldon_action_before_main_js', $main_js_dependency );

			// Enqueue theme's main script
			wp_enqueue_script( 'eldon-main-js', ELDON_ASSETS_JS_ROOT . '/main.min.js', $main_js_dependency, false, true );

			// Hook to include additional scripts after theme's main script
			do_action( 'eldon_action_after_main_js' );
		...
		function localize_js_scripts() {
			$global = apply_filters(
				'eldon_filter_localize_main_js',
				array(
					'adminBarHeight' => is_admin_bar_showing() ? 32 : 0,
					'iconArrowLeft'  => eldon_get_svg_icon( 'slider-arrow-left' ),
					'iconArrowRight' => eldon_get_svg_icon( 'slider-arrow-right' ),
					'iconClose'      => eldon_get_svg_icon( 'close' ),
				)
			);
		...
		function include_plugins_styles() {
			...
			wp_enqueue_style( 'swiper', ELDON_ASSETS_ROOT . '/plugins/swiper/swiper.min.css' );
		}

		function include_plugins_scripts() {

			// JS dependency variables
			$js_3rd_party_dependency = apply_filters( 'eldon_filter_js_3rd_party_dependency', 'jquery' );

			// Enqueue 3rd party plugins script
			wp_enqueue_script( 'jquery-waitforimages', ELDON_ASSETS_ROOT . '/plugins/waitforimages/jquery.waitforimages.js', array( $js_3rd_party_dependency ), false, true );
			wp_enqueue_script( 'swiper', ELDON_ASSETS_ROOT . '/plugins/swiper/swiper.min.js', array( $js_3rd_party_dependency ), false, true );
			wp_enqueue_script( 'gsap', ELDON_ASSETS_ROOT . '/plugins/gsap/gsap.min.js', array( $js_3rd_party_dependency ), false, true );
		}
		...
		function add_skip_link() {
			echo '<a class="skip-link screen-reader-text" href="#qodef-page-content">' . esc_html__( 'Skip to the content', 'eldon' ) . '</a>';
		}
		...
		function include_google_fonts() {
			...
			$font_weight_str = implode( ',', array_unique( apply_filters( 'eldon_filter_google_fonts_weight_list', $font_weight_array ) ) );
			$font_subset_str = implode( ',', array_unique( apply_filters( 'eldon_filter_google_fonts_subset_list', $font_subset_array ) ) );
			$fonts_array     = apply_filters( 'eldon_filter_google_fonts_list', $default_font_family );
			...
			if ( ! empty( $fonts_array ) ) {
				...
				wp_enqueue_style( 'eldon-google-fonts', esc_url_raw( $google_fonts_url ), array(), '1.0.0' );
			}
		}
		...
		function set_theme_support() {
			...
			load_theme_textdomain( 'eldon', ELDON_ROOT_DIR . '/languages' );
			...
			if ( ! isset( $content_width ) ) {
				$content_width = apply_filters( 'eldon_filter_set_content_width', 1300 );
			}
			...
			add_editor_style( ELDON_ASSETS_CSS_ROOT . '/editor-style.min.css' );
		}
		...
		function editor_customizer_styles() {
			...
			wp_enqueue_style( 'eldon-editor-customizer-styles', ELDON_ASSETS_CSS_ROOT . '/editor-customizer-style.css' );

			// Add Gutenberg blocks style
			wp_enqueue_style( 'eldon-gutenberg-blocks-style', ELDON_INC_ROOT . '/gutenberg/assets/admin/css/gutenberg-blocks.css' );
		}
		...
		function add_body_classes( $classes ) {
			...
			return apply_filters( 'eldon_filter_add_body_classes', $classes );
		}

		function include_modules() {

			// Hook to include additional files before modules inclusion
			do_action( 'eldon_action_before_include_modules' );

			foreach ( glob( ELDON_INC_ROOT_DIR . '/*/include.php' ) as $module ) {
				include_once $module; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			}

			// Hook to include additional files after modules inclusion
			do_action( 'eldon_action_after_include_modules' );
		}
	}

	Eldon_Handler::get_instance();
}
